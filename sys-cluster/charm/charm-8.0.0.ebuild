...
get_opts() {
	...
	# TLS is only needed by AMPI, which we don't build, and breaks dlopen.
	CHARM_OPTS+=" --disable-tls"
...
src_prepare() {
	...
	# Build system tries to build it even though it isn't requested, and
	# fails.
	eapply "${FILESDIR}/charm-8.0.0-disable-ampi.patch"
...
src_install() {
	...
	eapply "${FILESDIR}/charm-8.0.0-charmc-gentoo.patch"
