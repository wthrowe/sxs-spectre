...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="charmdebug charmtracing cmkopt examples isomalloc mlogft mpi ampi numa smp static-libs syncft tcp udp"
...
src_prepare() {
	...
	use isomalloc || append-cflags -DCMK_NO_ISO_MALLOC
...
src_compile() {
	...
	if use ampi; then
		...
		./build AMPI ${build_commandline} || die "Failed to build AMPI"
	fi
}
